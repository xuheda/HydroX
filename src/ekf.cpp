...
namespace hydrox
{
    ...
    EKF::EKF(const Params &p)
    {
        ...
        q_diag << p.q_pos, p.q_pos, p.q_pos,
            p.q_att, p.q_att, p.q_att,
            p.q_vel, p.q_vel, p.q_vel,
            p.q_ba, p.q_ba, p.q_ba,
            p.q_bg, p.q_bg, p.q_bg,
            p.q_current, p.q_current, p.q_current;
        _Q = q_diag.asDiagonal();

        _R_dvl = p.r_dvl * Eigen::Matrix3d::Identity();
        _R_water_dvl = p.r_water_dvl * Eigen::Matrix3d::Identity();
        ...
        _gate_water_dvl_nis = p.gate_water_dvl_nis;
        ...
        _initial_current_variance = std::max(p.initial_current_variance, _min_variance);
        _current_valid_std = std::max(p.current_valid_std, 0.0);
        _x.setZero();
        _P = Eigen::Matrix<double, EKF_N, EKF_N>::Identity();
        _P.block<3, 3>(15, 15) =
            _initial_current_variance * Eigen::Matrix3d::Identity();
    }

    void EKF::reset(const AUVState &init)
    {
        _x.setZero();
        ...
        // Bias (9..14) and NED water current (15..17) initialized to 0.
        _P = Eigen::Matrix<double, EKF_N, EKF_N>::Identity();
        _P.block<3, 3>(15, 15) =
            _initial_current_variance * Eigen::Matrix3d::Identity();
    ...
    AUVState EKF::update(const NavigationMeasurements &meas,
                         ...
                         double dt)
    {
        ...
        // Perform accelerometer leveling measurement when specific force is available, making roll/pitch and horizontal gyro bias observable
        if (have_accel)
        {
            if (_update_level(accel_body, _valid_cov3(meas.accel_body.covariance, _R_level)))
                ++_last_stats.level_accepted;
            else
                ++_last_stats.level_rejected;
        }

        const bool bottom_dvl_available =
            bottom_dvl && bottom_dvl->beam_valid > 0 && meas.dvl_velocity_body.meta.valid;
        const bool water_dvl_available =
            water_dvl && water_dvl->beam_valid > 0 && meas.water_dvl_velocity_body.meta.valid;

        if (bottom_dvl_available &&
            _consume_new_sample(bottom_dvl->timestamp, _last_bottom_dvl_timestamp_s))
        {
            if (_update_dvl(bottom_dvl->velocity_body(),
            ...
            _consume_new_sample(water_dvl->timestamp, _last_water_dvl_timestamp_s))
        {
            ...
            _update_zvu(); // kinematic fallback: weakly hold velocity near zero without DVL

        if (meas.depth.meta.valid)
        {
            if (_update_depth(meas.depth.value,
        ...
        const bool gps_new =
            gps && meas.gps_position_ned.meta.valid &&
        ...
        if (gps_new)
        {
            ...
            if (gps->has_altitude)
            {
                if (_update_gps_altitude(
                        gps->pos_d,
                        _valid_variance(meas.gps_position_ned.covariance(2, 2), _R_gps_z(0, 0))))
        ...
        _x[5] = psi_pred;
        _x[14] = bgz_pred;
        const std::optional<double> mag_heading =
            meas.mag_body.meta.valid ? _heading_from_magnetometer(meas.mag_body.value) : std::nullopt;
        if (mag_heading)
        {
            if (_update_heading(*mag_heading, _heading_variance_from_mag(meas.mag_body)))
        ...
        else if (meas.truth_heading_debug.meta.valid)
        {
            if (_update_heading(
                    ...
                    _valid_variance(meas.truth_heading_debug.variance, _R_heading(0, 0))))
                ++_last_stats.heading_accepted;
            else
                ++_last_stats.heading_rejected;
        }
        ...
        out.nu.segment<3>(3) = omega_body - _x.segment<3>(12); // De-biased gyro angular velocity
        out.depth_m = _x[2];
        out.current_ned = _x.segment<3>(15);
        for (int axis = 0; axis < 3; ++axis)
        {
            out.current_std_ned[axis] = std::sqrt(
                std::max(_min_variance, _P(15 + axis, 15 + axis)));
        }
        out.current_valid =
            (out.current_std_ned.array() <= _current_valid_std).all();
        out.dvl_valid = bottom_dvl.has_value();
        return out;
    }

    // Continuous-time dynamics x_dot = f(x, accel, omega)
    Eigen::Matrix<double, EKF_N, 1>
    EKF::_dynamics(const Eigen::Matrix<double, EKF_N, 1> &x,
                   ...
                   bool have_accel)
    {
        ...
        if (have_accel)
        {
            ...
            // v_dot_b = f + R_nb^T g_n - omega x v_b (when static and level, f = [0, 0, -g] -> v_dot = 0)
            xdot.segment<3>(6) = f + R.transpose() * g_n - omega_c.cross(v_b);
        }
        // have_accel=false -> velocity is not integrated via specific force (driven by DVL/ZVU updates), degrading to kinematics

        // Bias random walk: mean derivative is 0
        return xdot;
    }

    // Prediction step: state Euler integration + numerical Jacobian covariance propagation
    void EKF::_predict(const Eigen::Vector3d &accel, const Eigen::Vector3d &omega,
                       bool have_accel, double dt)
    {
        const Eigen::Matrix<double, EKF_N, 1> f0 = _dynamics(_x, accel, omega, have_accel);

        // Numerical Jacobian A = df/dx (central difference is slightly expensive, forward difference is sufficient)
        ...
        for (int j = 0; j < EKF_N; ++j)
        {
            ...
            xp[j] += eps;
            const Eigen::Matrix<double, EKF_N, 1> fp = _dynamics(xp, accel, omega, have_accel);
            A.col(j) = (fp - f0) / eps;
        }
        ...
        const Eigen::Matrix3d &R)
    {
        ...
        // The attitude term is nonlinear. Numerically differentiate only the
        // three Euler columns; the velocity/current blocks are exact.
        const double eps = 1e-6;
        for (int j = 3; j <= 5; ++j)
        {
            Eigen::Matrix<double, EKF_N, 1> xp = _x;
    ...
    bool EKF::_update_gps_velocity(const GPSMeasurement &gps, const Eigen::Matrix3d &R)
    {
    ...
    std::optional<double> EKF::_heading_from_magnetometer(
        const Eigen::Vector3d &mag_body) const
    {
        ...
        const double leveled_x =
            ctheta * mag_body.x() + stheta * (sphi * mag_body.y() + cphi * mag_body.z());
    ...
    {
        // Only enabled when nearly static or moving at constant speed (no significant linear acceleration): |norm(f_meas) - g| <= threshold
        if (std::abs(accel.norm() - EKF_G) > _level_gate)
            return false;

        const double phi = _x[3], theta = _x[4], psi = _x[5];
        // Predicted specific force (static leveling model) h = -R_nb^T g_n = -gravity_body
        ...
        const Eigen::Vector3d y = (accel - _x.segment<3>(9)) - h; // Measurement = de-biased specific force
        const Eigen::Matrix3d S = H * _P * H.transpose() + R;
        const double nis = (y.transpose() * S.inverse() * y)(0, 0);
    ...
    Eigen::Matrix3d EKF::_valid_cov3(const Eigen::Matrix3d &R, const Eigen::Matrix3d &fallback) const
    {
    ...
    Eigen::Matrix2d EKF::_valid_cov2(const Eigen::Matrix2d &R, const Eigen::Matrix2d &fallback) const
    {
    ...
    Eigen::Matrix3d EKF::_J2(double phi, double theta)
    {
        ...
        if (std::abs(cth) < 1e-6)
            cth = (cth < 0 ? -1e-6 : 1e-6);
        Eigen::Matrix3d J;
        J << 1.0, sphi * sth / cth, cphi * sth / cth,
            0.0, cphi, -sphi,
